				...
				<div id="panier" class="container tab-pane active">
					...
					<section class="content-holder">
						{foreach from=$list_offer item=value} 
							<div class="container container_middle">
								<h2 class="name-page">{$value.offer}</h2>
								<div class="content">
									<div class="item-proposal description-content">
										<div class="img-holder">
										<img src="{$value.logo}" alt="Opperateur"/>
										</div>
										<p>{$value.supplier}  {$value.technologie} </p>
								...
								<aside class="sidebar">
									<p class="price price_orange"><a href="{$value.link}" target="_blank">Passer Commande</a></p>
									<div class="button btn-lightblue">{$value.price} DH/mois</div>
									{if $value.promotion!=0}
									<div class="button btn-lightblue"><strike>{$value.pricepromo}DH/mois</strike> pendant 12 mois</div> 
									{/if}
									<div class="user-info">
										...
										<p class="icon">
											...
											{$value.supplier}  {$value.technologie} 	
										</p>
									</div>				
									<div class="user-info">
										<p class="icon">
											<svg class="icon-clock-new"><i class="fas fa-network-wired"></i></svg>
											<span class="characteristic_name">Technologie:</span> {$value.technologie}
										</p>
										{foreach from=$value.servcie item=value}
											<p class="icon">
													<svg class="icon-clock-new"><i class="{$value.icon}"></i></svg>
													<span class="characteristic_name">{$value.type}:</span> {$value.service}
											</p>
										{/foreach}							
										<p class="icon">
